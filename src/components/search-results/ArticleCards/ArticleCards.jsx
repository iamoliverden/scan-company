...
import mock_article_1_picture from '../../../assets/pics/mock_article_1_picture.png';
import mock_article_2_picture from '../../../assets/pics/mock_article_2_picture.png';
...
function ArticleCards({ documentsData }) {
  ...
  useEffect(() => {
    if (documentsData && Array.isArray(documentsData)) { 
      const transformedArticles = documentsData.map(doc => ({
        date: new Date(doc.ok.issueDate).toLocaleDateString("en-US"),
        url: doc.ok.url,
  ...
  const showMoreArticles = () => {
    setDisplayedArticles(prev => prev + 4); // show 4 more publications
  };

  return (
    <div className="article-cards-block">
      <h2 className="h2-search-results-page-articles">List of publications</h2>
      ...
      {displayedArticles < articles.length && (
        <div className="button-div">
            <button className="button show-more" onClick={showMoreArticles} >Show more</button>
        </div>
      )}
    </div>
  );
